    ...
    if(req.body.filter === "date"){
        ...
        await Hotel.find(function(err, hotels){
            if(err){
                ...
                hotels.map((hotel,i) => {
                        const room = hotel.room.filter((r) => {
                            return (
                                !moment(moment(r["from"]).format('YYYY-MM-DD')).isBetween(moment(range && range[0]).format('YYYY-MM-DD'),moment(range && range[1]).format('YYYY-MM-DD'),undefined,[])
                            )
                        })
                        hotel["room"] = room
                })
